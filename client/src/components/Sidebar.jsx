...
const Sidebar = ({ selectedUser, setSelectedUser,setUser,user }) => {
  ...
  useEffect(
    ()=>{
      const fetchUsers=async()=>{
        try {
          const res = await axios.get("https://velion-ea66.onrender.com/api/users",{
  ...
  return (
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-auto text-white ${selectedUser ? 'max-md:hidden' : ''}`}>
      <div className='pb-5'>
        ...
        <div className='flex rounded-full mt-5 bg-[#282142] gap-2 py-3 px-4 '>
      ...
      <div className='flex flex-col'>
        {users.map((user,index)=>(
          <div key={user._id} onClick={()=>setSelectedUser(user)}  className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id===user._id && 'bg-[#282142]/50'} `}>
            <img 
              src={user?.profilePic||assets.avatar_icon} alt='' className='w-8.75 aspect-square rounded-full'/>
